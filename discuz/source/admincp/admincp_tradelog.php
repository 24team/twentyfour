...
foreach(C::t('forum_tradelog')->fetch_all_by_status($filter, $start_limit, $ppp) as $tradelog) {
	...
	showtablerow('', '', array(
		$tradelog['tradeno'],
		'<a target="_blank" href="forum.htm?mod=viewthread&do=tradeinfo&tid='.$tradelog['tid'].'&pid='.$tradelog['pid'].'">'.$tradelog['subject'].'</a>',
		...
		$tradelog['credit'],
		$tradelog['tax'],
		'<a target="_blank" href="forum.htm?mod=trade&orderid='.$tradelog['orderid'].'&tid='.$tradelog['tid'].'&modthreadkey='.modauthkey($tradelog['tid']).'">'.$tradelog['status'].'<br />'.$tradelog['lastupdate']
	));
}
