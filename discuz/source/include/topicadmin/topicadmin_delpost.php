...
if(!($deletepids = dimplode($topiclist))) {
	...
	foreach(C::t('forum_post')->fetch_all('tid:'.$_G['tid'], $topiclist, false) as $post) {
		...
		if($post['first'] == 1) {
			dheader("location: $_G[siteurl]forum.htm?mod=topicadmin&action=moderate&operation=delete&optgroup=3&fid=$_G[fid]&moderate[]=$thread[tid]&inajax=yes".($_GET['infloat'] ? "&infloat=yes&handlekey={$_GET['handlekey']}" : ''));
		} else {
...
if(!submitcheck('modsubmit')) {
	...
	$resultarray = array(
	'redirect'	=> "forum.htm?mod=viewthread&tid=$_G[tid]&page=$_GET[page]",
