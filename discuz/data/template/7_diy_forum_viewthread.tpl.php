...
0
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/24style/forum/viewthread_node.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/24style/forum/viewthread_fastpost.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/24style/common/sidefoot.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/default/forum/viewthread_node_body.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/default/common/seditor.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/default/forum/seccheck_post.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/default/common/upload.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
|| checktplrefresh('./template/24style/forum/viewthread.htm', './template/default/common/seccheck.htm', 1582471945, 'diy', './data/template/7_diy_forum_viewthread.tpl.php', './template/24style', 'forum/viewthread')
...
<div id="ct" class="wp cl">
  ...
  <!--主题标题-->
      ...
      <h1 class="threadtitle"><?php echo $_G['forum_thread']['subject'];?></h1>
  <span class="xg1">
所在版块: <?php echo $_G['forum']['name'];?>
        <?php echo date("Y-m-d H:i",$_G['forum_thread']['dateline']);; ?>    <?php if($_G['forum_thread']['displayorder'] == -2) { ?>(审核中)
...
&nbsp;<img src="<?php echo IMGDIR;?>/recommend_<?php echo $_G['forum_thread']['recommendlevel'];?>.gif" alt="" title="{
lang thread_recommend} <?php echo $_G['forum_thread']['recommends'];?>" />
<?php } if($_G['forum_thread']['heatlevel']) { ?>
&nbsp;<img src="<?php echo IMGDIR;?>/hot_<?php echo $_G['forum_thread']['heatlevel'];?>.gif" alt="" title="热度: 
<?php echo $_G['forum_thread']['heats'];?>" />
<?php } if($_G['forum_thread']['closed'] == 1) { ?>
&nbsp;<img src="<?php echo IMGDIR;?>/locked.gif" alt="关闭" title="关闭" class="vm" />
<?php } ?>
<a href="forum.php?mod=viewthread&amp;tid=<?php echo $_G['tid'];?><?php echo $fromuid;?>" onclick="return copyThreadUrl(this, '<?php echo $_G;?>[
setting][bbname]')" <?php if($fromuid) { ?>title="您的朋友访问此链接后，您将获得相应的积分奖励"<?php } ?>>[复制链接]</a>
  </span>
...
<?php } ?>
  ...
  <?php $postcount = 0;?>      <?php if(is_array($postlist)) foreach($postlist as $post) { ?>        <?php if($rushreply && $_GET['checkrush'] && $post['rewardfloor'] != 1) { ?>
            <?php continue;?>        <?php } ?>
        <div id="post_<?php echo $post['pid'];?>" <?php if($_G['blockedpids'] && $post['inblacklist']) { ?>style="display:none;"<?php } ?> 
...
<table id="pid<?php echo $post['pid'];?>" class="plhin" summary="pid<?php echo $post['pid'];?>" cellspacing="0" cellpadding="0" width="100%">
<tr>
<?php if(!$post['first']) { ?>
...
<?php } ?>
<div class="avatardiv"<?php if(!($_G['setting']['threadguestlite'] && !$_G['uid'])) { ?> onmouseover="showauthor(this, 'userinfo<?php echo $post['pid'];?>')"<?php } ?>><a href="home.php?mod=space&amp;uid=<?php echo $post['authorid'];?>" class="avtm" target="_blank"><?php echo $post['avatar'];?></a></div>
<p class="authorname_div"><?php echo $post['author'];?></p>
<?php } ?>
...
<div class="pi" <?php if($post['first']) { ?>style="display:none"<?php } ?>>
...
<?php if(!$_G['forum_thread']['is_archived']) { ?>
        <a id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"<?php if(!$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']) { ?> onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=<?php echo $_G['fid'];?>')"<?php } else { ?> onclick="location.href='forum.php?mod=post&action=newthread&fid=<?php echo $_G['fid'];?>';return false;"<?php } ?> href="javascript:;" title="发新帖" class="mwt-btn mwt-btn-primary" style="width:40px;">发新帖</a>
<?php } if($allowpostreply && !$_G['forum_thread']['archiveid']) { ?>
        <a id="post_replytmp" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=<?php echo $_G['fid'];?>&tid=<?php echo $_G['tid'];?>')" href="javascript:;" title="回复" class="mwt-btn mwt-btn-primary" style="width:40px;">回复</a>
<?php } ?>
      </div>
...
<tr>
<td class="pls">
<?php if($_G['uid']) { ?><div class="avatardiv" style="margin-top:15px;"><?php echo avatar($_G['uid']); ?></div><?php } ?>
...
<?php if(!$_G['forum_thread']['is_archived']) { ?>
        <a id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"<?php if(!$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']) { ?> onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=<?php echo $_G['fid'];?>')"<?php } else { ?> onclick="location.href='forum.php?mod=post&action=newthread&fid=<?php echo $_G['fid'];?>';return false;"<?php } ?> href="javascript:;" title="发新帖" class="mwt-btn mwt-btn-primary" 
        style="width:<?php if($allowpostreply && !$_G['forum_thread']['archiveid']) { ?>70<?php } else { ?>180<?php } ?>px;">发新帖</a>
<?php } if($allowpostreply && !$_G['forum_thread']['archiveid']) { ?>
        <a id="post_replytmp" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=<?php echo $_G['fid'];?>&tid=<?php echo $_G['tid'];?>')" href="javascript:;" title="回复" class="mwt-btn mwt-btn-primary" style="width:70px;margin-left:10px;">回复</a>
<?php } ?>
...
var params={
fid:'<?php echo $_G['forum_thread']['fid'];?>',
tid:'<?php echo $_G['forum_thread']['tid'];?>',
authorid: '<?php echo $_G['forum_thread']['authorid'];?>'
};
