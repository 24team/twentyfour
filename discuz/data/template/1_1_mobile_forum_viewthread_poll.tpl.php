...
<div class="bt pd2">
<form id="poll" name="poll" method="post" autocomplete="off" action="forum.php?mod=misc&amp;action=votepoll&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;pollsubmit=yes<?php if($_GET['from']) { ?>&amp;from=<?php echo $_GET['from'];?><?php } ?>&amp;quickforward=yes&amp;mobile=yes" >
...
<div>
        <?php if(is_array($polloptions)) foreach($polloptions as $key => $option) { ?>            <p>
            <?php if($_G['group']['allowvote']) { ?>
                <input type="<?php echo $optiontype;?>" id="option_<?php echo $key;?>" name="pollanswers[]" value="<?php echo $option['polloptionid'];?>" <?php if($_G['forum_thread']['is_archived']) { ?>disabled="disabled"<?php } ?>  />
            <?php } ?>
            <label for="option_<?php echo $key;?>"><?php echo $key;?>.<?php echo $option['polloption'];?></label>
            <?php if(!$visiblepoll) { ?>
                <?php echo $option['percent'];?>% <em style="color:#<?php echo $option['color'];?>">(<?php echo $option['votes'];?>)</em>
            <?php } ?>
            </p>
        <?php } ?>
