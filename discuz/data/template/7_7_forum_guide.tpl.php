...
0
|| checktplrefresh('./template/24style/forum/guide.htm', './template/default/forum/guide_list_row.htm', 1585728434, '7', './data/template/7_7_forum_guide.tpl.php', './template/24style', 'forum/guide')
;?>
<?php if($_GET['inajax']) { ?>
  ...
  show_user_space_card(id,uid);
    ...
    <ul class="threads-ul"><?php $i=0;?>    <?php if(is_array($data)) foreach($data as $key => $list) { ?>      <?php if($list['threadcount']) { ?>
    <?php if(is_array($list['threadlist'])) foreach($list['threadlist'] as $thread) { ?>        <?php ++$i;?><li>
<div class="theme_list_img">
      <a id="useravatar-<?php echo $i;?>" name="useravatar" data-uid="<?php echo $thread['authorid'];?>" href="home.php?mod=space&amp;uid=<?php echo $thread['authorid'];?>" 
         target="_blank" rel="noopener noreferrer"
         onmouseover="pop_user_space_card('useravatar-<?php echo $i;?>');";
 style="display:block;"><img class="user_head" src="<?php echo avatar($thread[authorid],'middle',true);?>" style="display: block;"></a>
</div>
...
<?php } ?>
  ...
  <div class="auth_msg mwt-border-bottom">
    <a href="home.php?mod=space&amp;uid=<?php echo $thread['authorid'];?>" class="user_name" target="_blank" rel="noopener noreferrer"><?php echo $thread['author'];?></a>&nbsp;
<span><?php echo $thread['dateline'];?></span>&nbsp;
<a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $thread['fid'];?>" class="time txt"><?php echo $list['forumnames'][$thread['fid']]['name'];?></a>
<span style="float:right;">
  <span class="numb"><i style="display:none;" class="icon icon-preview"></i>查看: <?php echo $thread['views'];?></span>&nbsp;&nbsp;
  <span class="numb"><i style="display:none;" class="icon icon-comment"></i>回复: <?php echo $thread['replies'];?></span>
</span>
  </div>
</div>
  </li>
  ...
  <?php include template('common/footer'); ?>  <?php return;?><?php } include template('common/header'); ?><style type="text/css">
...
<?php } } else { if(is_array($data)) foreach($data as $key => $list) { ?><div id="threadlist" class="tl bm bmw"<?php if($_G['uid']) { ?> style="position: relative;padding:10px 20px;border:solid 1px #e6e6e6;border-bottom:none;"<?php } ?>>
<div class="th">
